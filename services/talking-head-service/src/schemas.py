...
class JobStatusResponse(BaseModel):
    ...
    progress: float = Field(0.0, description="Job completion progress percentage (0-100)")
    estimated_time_remaining: float = Field(0.0, description="Estimated time remaining in seconds")
    created_at: str = Field(..., description="ISO timestamp of job creation")
    completed_at: Optional[str] = Field(None, description="ISO timestamp of job completion or failure")
    output_url: Optional[str] = Field(None, description="URL of generated video output if completed")
    error_message: Optional[str] = Field(None, description="Details of failure if job failed")
