...
def prepare_job_storage(job_id: str) -> Path:
    ...
    try:
        ...
        raise StorageError(f"Failed to initialize storage directory for job '{job_id}'.")

def save_job_inputs(job_id: str, image_filename: str, image_bytes: bytes, audio_filename: str, audio_bytes: bytes) -> dict:
    ...
    try:
        ...
        logger.info(f"Saved job inputs for {job_id}: image='{img_path}' ({len(image_bytes)} bytes), audio='{audio_path}' ({len(audio_bytes)} bytes)")
