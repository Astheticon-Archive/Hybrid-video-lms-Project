...
class ServiceValidationError(ServiceBaseException):
    def __init__(self, detail: str, error_code: str = "VALIDATION_ERROR", status_code: int = status.HTTP_400_BAD_REQUEST):
...
class StorageError(ServiceBaseException):
    def __init__(self, detail: str = "Failed to store uploaded files safely."):
        super().__init__(detail, error_code="STORAGE_ERROR", status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

class PipelineError(ServiceBaseException):
    def __init__(self, detail: str = "Talking Head pipeline processing failed."):
        super().__init__(detail, error_code="PIPELINE_ERROR", status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

class JobNotFoundError(ServiceBaseException):
    def __init__(self, job_id: str):
        super().__init__(f"Job '{job_id}' not found.", error_code="JOB_NOT_FOUND", status_code=status.HTTP_404_NOT_FOUND)

def register_exception_handlers(app: FastAPI):
    
    @app.exception_handler(ServiceBaseException)
    async def custom_base_exception_handler(request: Request, exc: ServiceBaseException):
        logger.warning(f"Handled custom exception: code={exc.error_code} detail={exc.detail} status={exc.status_code}")
        return JSONResponse(
