...
export default makeScene2D('scene11', function* (view) {
  ...
  view.add(
    <Background>
      <Rect ref={cameraRef} size={['100%', '100%']} justifyContent={'center'} alignItems={'center'}>
        ...
        {/* User Card */}
        <Card ref={userCardRef} x={-750} y={0} width={160} height={120} opacity={1}>
        ...
        {/* Vector */}
        <Vector ref={vectorCardRef} x={-480} y={0} opacity={1} scale={0.7} values={[0.82, -0.41, 0.09]} />

        {/* Database */}
        <Rect ref={dbContainerRef} x={-180} y={0} opacity={1} scale={0.7}>
          <Database label={'V-DB'} />
        </Rect>

        {/* Document */}
        <Rect ref={docContainerRef} x={120} y={0} opacity={1} scale={0.65}>
          <Document highlightedLine={1} />
        </Rect>

        {/* Prompt */}
        <Rect ref={promptContainerRef} x={420} y={0} opacity={1} scale={0.55}>
          <PromptBox questionText={'Query?'} contextText={'Facts...'} />
        </Rect>

        {/* LLM */}
        <Card ref={llmCardRef} x={740} y={0} width={180} height={120} opacity={1} glowColor={THEME.colors.purple}>
        ...
        <Rect ref={successBadgeRef} x={740} y={110} opacity={1}>
  ...
  yield* all(
    ...
    chain(
      typeText(captionTxt, 'By connecting query embeddings directly to a vector database, RAG grounds the LLM in real-world facts.', 8.04)
    )
  );
});
